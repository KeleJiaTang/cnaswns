...
{
    public partial class HCSCM_sterilizer_device_program : TemplateForm
    {
        ...
        public HCSCM_sterilizer_device_program(SortedList slselect)
        {
            InitializeComponent();
            ...
            dtall = reCnasRemotCall.RemotInterface.SelectData("HCS-sterilizer-program-sec001", null);
            SortedList sttemp02 = new SortedList();
            sttemp02.Add(1, slselect["id"].ToString());
            dtselect = reCnasRemotCall.RemotInterface.SelectData("HCS-sterilizer-deviceprogram-sec001", sttemp02);
            if (dtall != null)
            {
        ...
        private void but_save_Click(object sender, EventArgs e)
        {
            if (dgv_02.Rows.Count > 0)
            {
                ...
                int recint = reCnasRemotCall.RemotInterface.UPData(3, "HCS-sterilizer-deviceprogram-add001", sltmp01, sltmp02);
                if (recint > -1)
                {
                    MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("successful",EnumPromptMessage.warning,new string[]{"灭菌器关联程序"}),"提示信息",MessageBoxButtons.OK,MessageBoxIcon.Information);
                    this.Close();
                }
            }
            else
            {
               ...
               // string recsql = reCnasRemotCall.RemotInterface.CheckUPData(3, "HCS-sterilizer-deviceprogram-del001", sltmp01, null);
                int recint = reCnasRemotCall.RemotInterface.UPData(1, "HCS-sterilizer-deviceprogram-del001", sltmp01, null);
                if (recint > -1)
                {
                    MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("successful", EnumPromptMessage.warning, new string[] { "灭菌器关联程序" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
