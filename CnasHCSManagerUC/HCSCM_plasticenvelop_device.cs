...
{
    public partial class HCSCM_plasticenvelop_device : UserControl
    {
        ...
        public HCSCM_plasticenvelop_device()
        {
            InitializeComponent();
            #region 按钮图片加载
            ...
            this.but_pla_program.Image = ResourcesImageHelper.Instance.GetResourcesImage("Common.Buttom", "program", EnumImageType.PNG);
            this.but_pla_procedure.Image = ResourcesImageHelper.Instance.GetResourcesImage("Common.Buttom", "relatedProgram", EnumImageType.PNG);
           ...
           // this.Font = new Font(this.Font.FontFamily, 11);
            ...
            if (getdt != null)
            {
                ...
                //获取打印 BCT 条码的 XML 数据信息
                DataRow[] arrayDR02 = CnasBaseData.SystemBaseData.Select("type_code='HCS_barcode_type' and key_code='BCW'");
       ...
       /// </summary>
        private void Loaddata()
        {
            ...
            DataTable getdt = reCnasRemotCall.RemotInterface.SelectData("HCS-plasticenvelop-device-sec001", sttemp01);
            if (getdt != null && getdt.Rows.Count>0)
            {
                ...
                for (int i = 0; i < ii; i++)
                {
					...
					if (getdt.Columns.Contains("if_bdtest") && getdt.Rows[i]["if_bdtest"] != null) dgv_01.Rows[i].Cells["if_bdtest"].Value = (getdt.Rows[i]["if_bdtest"].ToString() == "1") ? "需要" : "不需要"; ;
					if (getdt.Columns.Contains("ca_model") && getdt.Rows[i]["ca_model"] != null) dgv_01.Rows[i].Cells["ca_model"].Value = getdt.Rows[i]["ca_model"].ToString();
					if (getdt.Columns.Contains("run_times") && getdt.Rows[i]["run_times"] != null) dgv_01.Rows[i].Cells["run_times"].Value = getdt.Rows[i]["run_times"].ToString();
					...
					if (getdt.Columns.Contains("price") && getdt.Rows[i]["price"] != null) dgv_01.Rows[i].Cells["price"].Value = double.Parse(getdt.Rows[i]["price"].ToString()).ToString("C");
					...
					if (getdt.Columns.Contains("bd_test_time") && getdt.Rows[i]["bd_test_time"] != null) dgv_01.Rows[i].Cells["bd_test_time"].Value = getdt.Rows[i]["bd_test_time"].ToString();
					if (getdt.Columns.Contains("sales_id") && getdt.Rows[i]["sales_id"] != null) dgv_01.Rows[i].Cells["sales_id"].Value = sl_SalesType[getdt.Rows[i]["sales_id"].ToString()].ToString();
					if (getdt.Columns.Contains("ca_vender") && getdt.Rows[i]["ca_vender"] != null) dgv_01.Rows[i].Cells["ca_vender"].Value = sl_VenderType[getdt.Rows[i]["ca_vender"].ToString()].ToString();
        ...
        {
            HCSCM_plasticenvelop_device_new hcsm = new HCSCM_plasticenvelop_device_new(null, getdt);
        ...
        private void but_edit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("choicechange", EnumPromptMessage.warning, new string[] { "修改的", "塑封机" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                ...
                HCSCM_plasticenvelop_device_new hcsm = new HCSCM_plasticenvelop_device_new(slindata, getdt);
                //获取一个值，指是否在Windows任务栏中显示窗体。
                hcsm.ShowInTaskbar = false;
                hcsm.ShowDialog();
                Loaddata();
        ...
        private void but_remove_Click(object sender, EventArgs e)
        {
            if (this.dgv_01.SelectedRows.Count <= 0)
            {
                MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("choicechange", EnumPromptMessage.warning, new string[] { "删除", "塑封机" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            try
            {
                CnasRemotCall reCnasRemotCall_01 = new CnasRemotCall();
                #region 判断塑封机机是否被塑封流程占用
                SortedList sttemp01 = new SortedList();
                sttemp01.Add(1, dgv_01.SelectedRows[0].Cells["id"].Value.ToString());
                DataTable getdt = reCnasRemotCall_01.RemotInterface.SelectData("HCS-plasticenvelop-deviceprogram-sec001", sttemp01);//99
                if (getdt != null)
                {
                    int ii = getdt.Rows.Count;
                    if (ii > 0)
                    {
                        for (int i = 0; i < ii; i++)
                        {
                            if (getdt.Rows[i]["dev_id"].ToString() != null)
                            {
                                ...
                                {
                                    MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("relation", EnumPromptMessage.warning, new string[] { "塑封机", "塑封流程", "塑封流程" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    return;
                                }
                            }
                        }
                ...
                #endregion
                    if (MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("choicedelete", EnumPromptMessage.warning, new string[] { dgv_01.SelectedRows[0].Cells["ca_name"].Value.ToString(), "塑封机" }), ConfigurationManager.AppSettings["SystemName"].ToString() + "--删除塑封机", MessageBoxButtons.YesNo) == DialogResult.No) return;
                    int selectedIndex = dgv_01.Rows.IndexOf(dgv_01.SelectedRows[0]);
                    ...
                    sltmp.Add(1, sltmp01);
                    CnasRemotCall reCnasRemotCall = new CnasRemotCall();
                    //string sql = reCnasRemotCall.RemotInterface.CheckUPData(1, "HCS-plasticenvelop-device-del001", sltmp, null);
                    int recint = reCnasRemotCall.RemotInterface.UPData(1, "HCS-plasticenvelop-device-del001", sltmp, null);
                    if (recint > -1)
                    {
                        MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("deletesuccessful", EnumPromptMessage.warning, new string[] { "塑封机" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void but_was_procedure_Click(object sender, EventArgs e)
        {
            if (this.dgv_01.SelectedRows.Count <= 0)
            {
                MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("choicechange", EnumPromptMessage.warning, new string[] { "关联", "塑封机" }), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                ...
                {
                    MessageBox.Show(PromptMessageXmlHelper.Instance.GetPromptMessage("fillprogram", EnumPromptMessage.warning, new string[] {"塑封机"}), "提示信息", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            }
            catch (Exception ex)
            ...
            HCSCM_plasticenvelop_device_procedure hcsm = new HCSCM_plasticenvelop_device_procedure(slselect);
            //获取一个值，指是否在Windows任务栏中显示窗体。
            hcsm.ShowInTaskbar = false;
            hcsm.ShowDialog();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void but_pla_program_Click(object sender, EventArgs e)
        {
            HCSCM_plasticenvelop_program hcsm = new HCSCM_plasticenvelop_program();
            //获取一个值，指是否在Windows任务栏中显示窗体。
            hcsm.ShowInTaskbar = false;
            hcsm.ShowDialog();
        }
        ...
        {
            DataRow[] arrayDR02 = CnasBaseData.SystemBaseData.Select("type_code='HCS-print-type' and key_code='plasticenvelop'");
