...
{
    public partial class HCSRS_washingno : TemplateForm
    {
        public HCSRS_washingno()
        {
            InitializeComponent();
        ...
        private void currentYearTxt_Click(object sender, EventArgs e)
        {
     ...
     /// </summary>
        ...
        private void rBut_ok_Click(object sender, EventArgs e)
        {
			...
			DataSet result = reCnasRemotCall.RemotInterface.ExecuteProcedure("clean_fail", bb);//根据时间查询返回所需要的数据
            ...
            if (result != null &&result.Tables != null && result.Tables.Count >0)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    int calYear = i==0 ? yearInt : yearInt - 1;//i=0时，等于今年；i=1时，等于上一年

                    //j表示哪一月
			        for (int j = 1; j < 13; j++)
			        {
			            string calMonth = calYear.ToString() + j.ToString().PadLeft(2,'0');//多少年多少月
