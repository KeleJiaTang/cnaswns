...
{
	public partial class HCSRS_outside_set : UserControl
	{
				...
				if (ids.Length >= 2)
				{
					...
					if (getdt01 != null)
					{
						...
						for (int i = 0; i < ii; i++)
						{
                            ...
                            if (getdt01.Columns.Contains("remark2") && !string.IsNullOrEmpty(getdt01.Rows[i]["remark2"].ToString())) dgv_01.Rows[i].Cells["remark"].Value = getdt01.Rows[i]["remark2"].ToString();
						}
						if(ii>0)
						{
