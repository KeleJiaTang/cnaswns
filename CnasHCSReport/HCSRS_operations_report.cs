...
{
    public partial class HCSRS_operations_report : UserControl
    {
        public HCSRS_operations_report()
        {
            InitializeComponent();
        ...
        private void radButton1_Click(object sender, EventArgs e)
        {
            ...
            #region 每月灭菌包装不合格率
            string strNore = "301,302,303,205";
            sttemp01.Add(1, strNore);
            sttemp01.Add(2, front);
            sttemp01.Add(3, back);
            string text = reCnasRemotCall.RemotInterface.CheckSelectData("HCS-statistics-sec001", sttemp01);
            DataTable getdtsum = reCnasRemotCall.RemotInterface.SelectData("HCS-statistics-sec001", sttemp01);
        ...
        private void radButton2_Click(object sender, EventArgs e)
        {
            ...
            int days = DateTime.DaysInMonth(int.Parse(comboBox1.Text.Trim()), month + 2);//季度最后月份的天数
        ...
        private void radButton3_Click(object sender, EventArgs e)
        {
            ...
            if (DSnorm != null)
            {
                ...
                if (cb_onlyyear.Text == dataNow.Year.ToString())
                {
                    ...
                    if (dataNow.Year.ToString() == cb_onlyyear.Text)
                    {
                        ...
                        if (dataNow.Day == LastDay)
                        {
                            ...
                            if (DSnorm.Tables[2].Rows[0]["SUM(instrument_count)"].ToString() == "")
                            {
                                norm.Add("每月平均处理外来器械的件数", 0 / dataNow.Month);
                            }
                            else
                            {
                                norm.Add("每月平均处理外来器械的件数", int.Parse(DSnorm.Tables[2].Rows[0]["SUM(instrument_count)"].ToString()) / dataNow.Month);
                            }
                            ...
                            int sum_wetbag = 0;
                            int sum_Sfailure = 0;
                            norm_1.Add("每月平均无菌包内器械缺失发生件数", 0);
                            norm_1.Add("每月平均无菌包内器械功能不全发生件数", 0);
                            norm_1.Add("每月平均无菌包内器械种类错误发生件数", 0);
                            norm_1.Add("每月平均无菌包标识不正确发生件数", 0);
                            norm_1.Add("每月平均灭菌包灭菌方式选择不正确发生件数", 0);
                            norm_1.Add("每月平均包内化学指标卡不合格件数", 0);
                            norm_1.Add("每月平均回收器械丢失发生件数", 0);
                            norm_1.Add("每月平均无菌物品发放错误发生件数", 0);
                            norm_1.Add("每月平均湿包发生件数", 0);
                            norm_1.Add("每月平均灭菌失败事件发生数", 0);
                            for (int i = 0; i < DSnorm.Tables[4].Rows.Count; i++)
                            {
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 209)//无菌包内器械功能不全发生件数
                                {
                                    norm_1["每月平均无菌包内器械缺失发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 210)//每月平均无菌包内器械缺失发生件数
                                {
                                    norm_1["每月平均无菌包内器械功能不全发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 103)//每月平均无菌包内器械种类错误发生件数
                                {
                                    norm_1["每月平均无菌包内器械种类错误发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 204)//每月平均无菌包标识不正确发生件数
                                {
                                    norm_1["每月平均无菌包标识不正确发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 205)//每月平均灭菌包灭菌方式选择不正确发生件数
                                {
                                    norm_1["每月平均灭菌包灭菌方式选择不正确发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 206)//每月平均包内化学指标卡不合格件数
                                {
                                    norm_1["每月平均包内化学指标卡不合格件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 102)//每月平均回收器械丢失发生件数
                                {
                                    norm_1["每月平均回收器械丢失发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 203)//每月平均无菌物品发放错误发生件数
                                {
                                    norm_1["每月平均无菌物品发放错误发生件数"] = int.Parse(DSnorm.Tables[4].Rows[i]["SUM(num)"].ToString()) / (dataNow.Month - 1);
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 207 || int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 208)//每月平均湿包发生件数,累加
                                {
                                    ...
                                    norm_1["每月平均湿包发生件数"] = sum_wetbag / 12;
                                    continue;
                                }
                                if (int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 301 || int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 302 || int.Parse(DSnorm.Tables[4].Rows[i]["type"].ToString()) == 303)//每月平均灭菌失败事件发生数,累加
                                {
                                    ...
                                    norm_1["每月平均灭菌失败事件发生数"] = sum_Sfailure / 12;
                                    continue;
                                }
                            }
                            #endregion
                        ...
                        else
                        {
                            ...
                            {
                                norm.Add("每月平均处理外来器械的件数", int.Parse(DSnorm.Tables[2].Rows[0]["SUM(instrument_count)"].ToString()) / (dataNow.Month - 1));
                            }
                ...
                else
                {
                    ...
                    {
                        norm.Add("每月平均处理外来器械的件数", int.Parse(DSnorm.Tables[2].Rows[0]["SUM(instrument_count)"].ToString()) / 12);
                    }
            ...
            windowsXPSeries.BackColor = Color.PapayaWhip;
            ...
            windowsXPSeries1.BackColor = Color.DeepSkyBlue;
            this.radChartView1.Series.AddRange(windowsXPSeries1);
            #endregion
        }
        private void radButton4_Click(object sender, EventArgs e)
        {
			cb_onlyyear.Text = cb_onlyyear2.Text;
			radButton3_Click(null,null);
        }
        ...
        {
            HCSRS_OutsideInstruments OI = new HCSRS_OutsideInstruments();
            OI.ShowDialog();
        }
